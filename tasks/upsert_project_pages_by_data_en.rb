...
projects.each_with_index do |project, index|
  ...
  path = "./_posts/english/#{project[:year]}-09-01-#{project[:id]}.md"
  page = <<~PROJECT_PAGE
    ---
    lang: en
    layout: post
    ...
    <nav>
      <p class='nav prev'><a href='#{prev_project[:id]}' title='#{prev_project[:title_en]}'>&larr; Prev<br>
        #{ Truncato.truncate(prev_project[:title_en]) }</a></p>
      <p class='nav next'><a href='#{next_project[:id]}' title='#{next_project[:title_en]}'>Next &rarr;<br>
