...
layout: post
title: |
  ベストエフォート型光回線を複数利用することで高速に通信を行うルータの開発
description: |
  複数の光回線を束ね、プライベートIP単位の動的負荷分散によりネットワーク全体の通信品質を向上させる広帯域ルータ。全回線の負荷を均等化し、既存の冗長化中心の制御では得にくい快適な体験を実現します。
permalink: /projects/2025/nextrouter
thumbnail: /assets/img/projects/2025/../../tbu.webp
...
<nav>
  <p class='nav prev'>
    <a href='aikyo' title='aikyo - 相互につながるAIキャラクターを作成するバックエンドフレームワーク'>
      ...
      {% else %}
        aikyo - 相互につながるAIキャラクターを作成するバックエン...
      {% endif %}
    </a>
  </p>
  ...
  <p class='nav next'>
    <a href='talkboost' title='TalkBoost - 吃音者の発話の自発性を促進し、積極的に話すことを支援するアプリ'>
      ...
      {% else %}
        TalkBoost - 吃音者の発話の自発性を促進し、積極的に話す...
      {% endif %}
    </a>
  </p>
</nav>
