...
description: |
  自然言語の指示からAIが編集エフェクトや必要なマスクを生成し、微調整用のパラメータUIも自動生成。自然言語の直感性とGUIの操作性を組み合わせ、スムーズな画像編集フローを実現します。
...
<nav>
  <p class='nav prev'>
    <a href='sms_solar' title='S.M.S.（Solar Management System）太陽の集光し家庭使用のために太陽光を電力、熱、光に変換し、屋内に伝達する機構'>
      &larr; {{ translations.navPrev[lang] }}
      <br>
      {% if page.lang == 'en' %}
        
      {% else %}
        S.M.S.（Solar Management System）太陽...
      {% endif %}
    </a>
  </p>
