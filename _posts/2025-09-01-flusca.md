...
layout: post
title: |
  Flusca - シミュレーションによって体験を構築するゲーム
description: |
  「Flusca（フラスカ）」は、自律的に変化し続けるシミュレーションによって、あなただけのプレイ体験を生み出すゲームです。あらかじめ定められた筋書きは存在せず、あなたの行動とキャラクターたちの自律的なふるまいが交わることで、主観的な解釈に基づく物語が頭の中に形作られていくでしょう。じっくりと眺め、時には直接関わってみてください。画面の向こうでは、今なにが起きていますか？
permalink: /projects/2025/flusca
thumbnail: /assets/img/projects/2025/flusca.png
...
<nav>
  <p class='nav prev'>
    <a href='uminavi' title='水中ドローンを使用した水中自己位置推定システム「UmiNavi」の開発'>
      &larr; {{ translations.navPrev[lang] }}
      <br>
      {% if page.lang == 'en' %}
        
      {% else %}
        水中ドローンを使用した水中自己位置推定システム「UmiNavi」の...
      {% endif %}
    </a>
  </p>
